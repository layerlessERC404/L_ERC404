...
abstract contract ERC404 is IERC404 {
  ...
  function burnForCrossChain(address from, uint256 amount) internal returns (uint256[] memory ids) {
    uint256 erc20BalanceOfSenderBefore = erc20BalanceOf(from);

    if (amount <= _minted) { // Intention is to transfer as ERC-721 token (id).
      
      uint256 id = amount;

      // Transfer 1 * units ERC-20 and 1 ERC-721 token.
      _transferERC20(from, address(0), units);
      _transferERC721(from, address(0), id);
      ids = new uint256[](1);
      ids[0] = id;
    } else {
     
        _transferERC20(from, address(0), amount);
        uint256 nftsToTransfer = amount / units;

      if(whitelist[from]){
        _minted+=nftsToTransfer;
      } else{
      ids = new uint256[](nftsToTransfer);
      for (uint256 i = 0; i < nftsToTransfer; i++) {
        ...
        _transferERC721(from, address(0), tokenId);
        ids[i]= tokenId;
      }
       // If the sender's transaction changes their holding from  a non-fractional to a fractional 
      // amount  adjust ERC-721s.
      //
      ...
      uint256 fractionalAmount = amount % units;
   ...
   function mintForCrossChain (address to,uint256 amount, uint256 minted, uint256[] memory ids) internal {
        uint256 erc20BalanceOfReceiverBefore = erc20BalanceOf(to);

          if(amount <= minted) {
            _transferERC20( address(0),to, units);
            _transferERC721(address(0), to, amount);
                ...
                if(ids.length != 0 ) {
                  for(uint256 i ; i<ids.length; i++ ){
                   _transferERC721(address(0), to, ids[i]);
                  }
                }else {
                  //this should only occur for whitelisted on mainchain
                  for(uint256 i ;i< nftsToTransfer; i++ ){
                   _transferERC721(address(0), to, minted +1 +i);
                  }
                }
  ...
  function _retrieveOrMintERC721(address to_) internal virtual {
    ...
    if (!DoubleEndedQueue.empty(_storedERC721Ids)) {
      ...
      if(!mainChain) revert OnlyMainChain();
      _minted++;
      id = _minted;
